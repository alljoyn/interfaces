...
The Controller Service can store only a maximum of 100 Pulse Effects or until the size of the 
persisted Pulse Effects file reaches 127KB whichever occurs first. If the user attempts to create more 
than 100 Pulse Effects, "no slot for new entry" is returned as the responseCode or if the user tries to
create a new Pulse Effect after the size of the Pulse Effect file reaches 127KB, "Not enough resources" is
...
Input arguments:
  ...
  * **toLampState** --- key value pairs --- the toState of the Pulse Effect. The keys are the property names in the [LampState interface](LampState-v1.xml) interface.
  ...
  * **fromLampState** --- key value pairs --- the fromState of the Pulse Effect. The keys are the property names in the [LampState interface](LampState-v1.xml) interface.
