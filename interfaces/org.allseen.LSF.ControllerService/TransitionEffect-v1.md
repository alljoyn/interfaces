...
Clients to create, update, delete, look up and apply Transition Effects on Lamps.
...
The Controller Service can store only a certian maximum number of Transition Effects or until the size of the 
persisted Transition Effects file reaches a certian specified value whichever occurs first. If the user attempts to create more 
than the allowed number of Transition Effects, "no slot for new entry" is returned as the responseCode or if the user tries to
create a new Transition Effect after the size of the Transition Effect file reaches the maximum allowed value, "Not enough resources" is
...
Input arguments:
  ...
  * **lampState** --- key value pairs --- the state of the Transition Effect. The keys are the property names 
