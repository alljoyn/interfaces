...
Output arguments:
  ...
  * **resultMessage** --- `string` --- Text that describes the connection
    result. The language of the text is English-only. _This is a legacy design
    and does not follow current interface design guidelines_.
...
The last error code and error message received from the underlying Wi-Fi layer.
  ...
  * **errorString** --- `string` --- Error message string. The language of the
    text is English-only. _This is a legacy design and does not follow current
    interface design guidelines_.
